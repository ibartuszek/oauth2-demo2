...
public class ExampleClientController {
    ...
    public ExampleResponse getTestResponse() {
        LOGGER.info("Client with access key sends request to resource server");
        ExampleResponse response = resourceServerClient.getExampleResponse();
        LOGGER.info("Client with access key received response from resource server: \'" + response.getMessage() + "\'");
        return response;
    }
